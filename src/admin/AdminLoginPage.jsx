...
import { FaUserShield, FaLock, FaEnvelope, FaEye, FaEyeSlash, FaChevronRight, FaArrowLeft, FaKey } from 'react-icons/fa';
          ...
          {resetMode ? (
            resetSent ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ width: 60, height: 60, borderRadius: 16, background: `${C.accent}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                  <FaEnvelope color={C.accent} size={26} />
                </div>
                <h3 style={{ color: C.dark, fontWeight: 800, fontSize: 22, marginBottom: 10 }}>Check Your Email</h3>
                <p style={{ color: C.muted, fontSize: 14, marginBottom: 28 }}>
                  If that email is registered, a reset link has been sent.
                </p>
                <button onClick={() => { setResetMode(false); setResetSent(false); setResetEmail(''); }}
                  style={{ padding: '12px 28px', borderRadius: 12, border: `1.5px solid ${C.border}`, background: 'transparent', color: C.accent, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
                  Back to Login
                </button>
              </div>
            ) : (
              <>
                <button onClick={() => setResetMode(false)} style={{ background: 'transparent', border: 'none', color: C.muted, cursor: 'pointer', fontSize: 13, fontWeight: 700, marginBottom: 20, padding: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <FaArrowLeft size={11} /> Back
                </button>
                <h2 style={{ color: C.dark, fontWeight: 800, fontSize: 24, marginBottom: 8 }}>Reset Password</h2>
                <p style={{ color: C.muted, fontSize: 14, marginBottom: 28 }}>Enter your admin email to receive a reset link.</p>
                <form onSubmit={handleReset}>
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'block', color: C.muted, fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10 }}>Email</label>
                    <div style={{ position: 'relative' }}>
                      <FaEnvelope style={{ position: 'absolute', left: 18, top: '50%', transform: 'translateY(-50%)', color: C.accentLt }} />
                      <input type="email" required value={resetEmail} onChange={e => setResetEmail(e.target.value)} placeholder="admin@email.com"
                        style={{ width: '100%', padding: '16px 16px 16px 50px', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 14, color: C.dark, fontSize: 16, outline: 'none' }} />
                    </div>
                  </div>
                  <button type="submit" disabled={resetLoading} style={{ width: '100%', padding: '16px', borderRadius: 14, border: 'none', background: C.dark, color: C.accentLt, fontWeight: 800, fontSize: 15, cursor: 'pointer', opacity: resetLoading ? 0.7 : 1 }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type={showPass ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)}
                      style={{ width: '100%', padding: '16px 50px 16px 50px', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 14, color: C.dark, fontSize: 16, outline: 'none' }} />
                ...
                <div style={{ textAlign: 'right', marginBottom: 24 }}>
                  <button type="button" onClick={() => setResetMode(true)}
                    style={{ background: 'transparent', border: 'none', color: C.accent, fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                    <FaKey size={11} /> Forgot Password?
                  </button>
                </div>
                ...
                <button type="submit" disabled={loading} style={{ width: '100%', padding: '18px', borderRadius: 14, border: 'none', background: C.dark, color: C.accentLt, fontWeight: 800, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: '0 8px 24px rgba(44,36,22,0.2)', opacity: loading ? 0.7 : 1 }}>
